...
export function TeacherAssignmentWorkspace() {
  ...
  return <main className="teacher-console">
    <header className="teacher-console__header"><div><p className="login-card__eyebrow">Teacher workspace</p><h1>Assignments</h1><p>Prepare work privately, then publish it to eligible students.</p></div><button className="teacher-console__primary" disabled={!scopes.length} onClick={() => showForm ? (setShowForm(false), setEditingAssignment(null)) : openCreateForm()} type="button">{showForm ? "Close form" : "Create assignment"}</button></header>
