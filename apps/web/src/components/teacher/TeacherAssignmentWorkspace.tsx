...
export function TeacherAssignmentWorkspace() {
  ...
  return <main className="teacher-console">
    <header className="teacher-console__header"><div><p className="login-card__eyebrow">Teacher workspace</p><h1>Assignments</h1><p>Prepare work privately, then publish it to eligible students.</p></div><button className="teacher-console__primary" disabled={!scopes.length} onClick={() => setShowForm((current) => !current)} type="button">{showForm ? "Close form" : "Create assignment"}</button></header>
    <p aria-live="polite" className="teacher-console__message">{message}</p>
    {showForm && <section className="teacher-panel teacher-panel--form"><h2>New draft assignment</h2><p>Scope and title save a Draft. Publishing requires every field.</p><form onSubmit={create}><label>Teaching scope<select defaultValue="" name="scope" required><option disabled value="">Choose Class/Course and Subject</option>{scopes.map((scope) => <option key={scope.classCourseId + scope.subjectId} value={scope.classCourseId + ":" + scope.subjectId}>{scope.classCourseName} · {scope.subjectName}</option>)}</select></label><label>Title<input maxLength={200} name="title" required /></label><label>Description<textarea maxLength={5000} name="description" rows={5} /></label><div className="teacher-form-grid"><label>Deadline<input name="deadline" type="datetime-local" /></label><label>Maximum marks<input min="0.01" name="maximumMarks" step="0.01" type="number" /></label></div><fieldset><legend>Allow updates before deadline?</legend><label><input name="updates" type="radio" value="yes" /> Yes</label><label><input name="updates" type="radio" value="no" /> No</label></fieldset><button type="submit">Save Draft</button></form></section>}
    <section className="teacher-panel"><div className="teacher-panel__toolbar"><div><h2>Your assignments</h2><p>Drafts are private; published work is visible to eligible students.</p></div><div className="teacher-tabs"><button className={view === "Draft" ? "is-active" : ""} onClick={() => setView("Draft")} type="button">Drafts</button><button className={view === "Published" ? "is-active" : ""} onClick={() => setView("Published")} type="button">Published</button></div></div><div className="teacher-assignment-list">{visibleAssignments.length === 0 ? <p className="teacher-empty">No {view.toLowerCase()} assignments yet.</p> : visibleAssignments.map((assignment) => <article className="teacher-assignment" key={assignment.id}><div><span className={assignment.status === "Published" ? "teacher-status teacher-status--published" : "teacher-status"}>{assignment.status}</span><h3>{assignment.title}</h3><p>{assignment.description || "No description added yet."}</p><small>{assignment.deadline ? "Deadline: " + new Date(assignment.deadline).toLocaleString() : "Deadline not set"} · {assignment.maximumMarks ? assignment.maximumMarks + " marks" : "Marks not set"}</small></div><div className="teacher-assignment__actions">{assignment.status === "Draft" ? <><button onClick={() => void action(assignment, "publish")} type="button">Publish</button><button className="teacher-assignment__subtle" onClick={() => void action(assignment, "delete")} type="button">Delete</button></> : <button className="teacher-assignment__subtle" onClick={() => void action(assignment, "unpublish")} type="button">Return to Draft</button>}</div></article>)}</div></section>
  </main>;
}
