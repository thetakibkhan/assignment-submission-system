...
import { getTeacherWorkspacePanelVisibility } from "@/components/teacher/teacher-workspace-view";
...
export function TeacherAssignmentWorkspace() {
  ...
  return <main className="teacher-console">
    <header className="teacher-console__header"><div><p className="login-card__eyebrow">Teacher workspace</p><h1>{reviewAssignment ? "Review submissions" : "Assignments"}</h1><p>{reviewAssignment ? reviewAssignment.title : "Prepare work privately, then publish it to eligible students."}</p></div><div className="teacher-console__actions"><SignOutButton className="workspace-sign-out" /><button className="teacher-console__primary" disabled={!reviewAssignment && !scopes.length} onClick={() => reviewAssignment ? closeReview() : showForm ? (setShowForm(false), setEditingAssignment(null)) : openCreateForm()} type="button">{reviewAssignment ? "Back to assignments" : showForm ? "Close form" : "Create assignment"}</button></div></header>
